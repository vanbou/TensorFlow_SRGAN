...
from tensorflow.python.keras.layers import Conv2D, BatchNormalization, Activation, Add, UpSampling2D, LeakyReLU, Dense
...
class SRGAN():
    ...
    def train(self, epochs ,init_epoch=0, batch_size=1, sample_interval=50):
        ...
        if init_epoch!= 0:
            self.generator.load_weights("weights/%s/gen_epoch%d.h5" % (self.dataset_name, init_epoch),skip_mismatch=False)
            self.discriminator.load_weights("weights/%s/dis_epoch%d.h5" % (self.dataset_name, init_epoch),skip_mismatch=False)
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            ...
            fig.savefig('images/%s/%d_high%d.png' % (self.dataset_name, epoch, i))
            plt.close()

if __name__ == '__main__':
    gan = SRGAN()
    gan.train(epochs=70000, init_epoch=50000, batch_size=1, sample_interval=50)
